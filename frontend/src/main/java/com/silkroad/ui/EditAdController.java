...
public class EditAdController {
    ...
    public void initialize() {
        ...
        categoryComboBox.valueProperty().addListener(
                (obs, oldValue, newValue) -> loadSubcategories(newValue));
    ...
    private void loadCategoriesAndCities() {
        try {
            ...
            e.printStackTrace();
            statusLabel.setText("Could not load categories.");
        }
        ...
        try {
            ...
            e.printStackTrace();
            statusLabel.setText("Could not load cities.");
        }
    }
    ...
    private void loadSubcategories(Category parent) {
        ...
        try {
            List<Category> subs = ApiClient.getSubcategories(parent.getId());
    ...
    private void prefillForm() {
        ...
        String adCategory = editingAd.getCategory();

        try {
            boolean found = false;

            for (Category parent : categoryComboBox.getItems()) {

                if (!parent.isHasChildren()) {

                    if (parent.getName().equalsIgnoreCase(adCategory)) {
                        categoryComboBox.setValue(parent);
                        found = true;
                        break;
                    }
                    ...
                    for (Category sub : subs) {
                        ...
                        if (sub.getName().equalsIgnoreCase(adCategory)) {

                            categoryComboBox.setValue(parent);

                            subCategoryComboBox.setItems(
                                    FXCollections.observableArrayList(subs));
        ...
        try {
            ...
            statusLabel.setText(
                    "Showing cached description — full details unavailable for non-approved ads.");
        }
    }
    ...
    private void handleSave() {
        ...
        Category parent = categoryComboBox.getValue();
        Category sub = subCategoryComboBox.getValue();
        Category categoryToSave = sub != null ? sub : parent;
        ...
        try {
            ...
            ApiClient.updateAd(
                    ...
                    categoryToSave != null ? categoryToSave.getId() : null,
